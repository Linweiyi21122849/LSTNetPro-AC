...
class REINFORCEAgent:
    def __init__(
        ...
        gamma: float = 0.99,  # 折扣因子，用于折扣回报值
        entropy_coef: float = 0.01, # 熵系数，用于控制策略的探索度，避免过早收敛到一个局部最优解
        device: str = "cpu",
        ...
        self.epochs = 5 # 一组数据训练10次，PPO是离线学习，特有参数
        self.eps = 0.2
        ...
        self.optimizer_actor = optim.RMSprop(self.model_actor.parameters(), lr=1e-3)
        self.optimizer_critic = optim.Adam(self.model_critic.parameters(), lr=1e-3)  # 值网络优化器
        
        self._state_buffer = []  # 记录状态
        self._action_buffer = [] # ppo算法 只记录动作就好，不用记录动作的log概率
        self._reward_buffer = []
    ...
    def update(self, obs: torch.Tensor, action: torch.Tensor, next_obs: torch.Tensor, reward: torch.Tensor, terminated: torch.Tensor, mask: torch.Tensor):
        """
        更新policy网络。
        ...
        self._action_buffer.append(action) # 策略概率分布
    ...
    def train_critic(self, obs, reward, next_obs, over):
        ...
        with torch.no_grad():
            target = self.model_critic(next_obs)
    ...
    def train_actor(self, state, action, td_delta, entropy, mask):
        ...
        # 计算优势函数
        for delta in torch.flip(td_delta, dims=[0]):  # 逆序时序差分值 axis=1轴上倒着取 [], [], []
            # 优势函数GAE的公式
        ...
        # 策略网络给出每个动作的logits，根据action得到当前时刻下该动作的对数概率
        logits = self.model_actor(state)
        # logits[mask == 0] = -float('inf')  # 设置无效动作的概率为负无穷
        policy_dist = Categorical(logits=logits)
        old_log_prob = policy_dist.log_prob(action).detach()  # 计算选定动作的对数概率
        ...
        for _ in range(self.epochs):
            ...
            # logits[mask == 0] = -float('inf')  # 设置无效动作的概率为负无穷
            policy_dist = Categorical(logits=logits)
            new_log_prob = policy_dist.log_prob(action)  # 计算选定动作的对数概率
            ...
            # 添加熵正则项
            # loss = -policy_loss
            actor_loss = -policy_loss + self.entropy_coef * policy_dist.entropy().mean()  # 熵项鼓励探索
    ...
    def _train(self):
        obs, action, reward, next_obs, over, entropy, mask  = self._buffer_to_tensor()
        obs = obs.squeeze(0)
        ...
        loss, entropy = self.train_actor(obs, action, td_error, entropy, mask)
 
        return loss, entropy
    
    def _buffer_to_tensor(self): # 将缓冲区中的数据转换为张量，返回奖励、动作对数概率和熵的堆叠张量。
        obs = torch.stack(self._state_buffer, dim=1).to(self.device)  # 获取状态
        action = torch.stack(self._action_buffer, dim=1).to(self.device)
        ...
        mask = torch.stack(self._mask_buffer, dim=1).to(self.device)
        ...
        # return reward, action, entropy, obs
        return  obs, action, reward, next_obs, over, entropy, mask
...
def train(env, agent: REINFORCEAgent, episodes: int, summary_freq: int):
    ...
    for e in range(episodes): # 玩episodes局游戏
        ...
        while not over:
            ...
            mmask = torch.tensor([mask], dtype=torch.bool)
