...
class REINFORCEAgent:
    def __init__(
        ...
        self.policy_network = PolicyNetwork(state_dim, item_dim).to(self.device)
        self.value_network = BaselineNetwork(state_dim).to(self.device) # 添加值网络

        self.optimizer = optim.RMSprop(self.policy_network.parameters(), lr=1e-5)
        self.value_optimizer = optim.Adam(self.value_network.parameters(), lr=1e-5)  # 值网络优化器
    ...
    def _train(self):
        reward, action_log_prob, entropy, obs = self._buffer_to_tensor() #先将缓冲区中的奖励、动作对数概率和熵转换为张量。
        obs = obs.squeeze(0)
        ...
        # 值网络损失 (MSE)
        baseline = self.value_network(obs)
        value_loss = F.mse_loss(baseline, ret)
        # 更新值网络
...
def inference(agent):
    ...
    plt.ylabel('Reward')
    plt.savefig(f"results/Reinforce/inference.png")
    plt.close()
    ...
    plt.xlabel('Time Stamp')
    plt.ylabel('Timecost')
    plt.savefig(f"results/Reinforce/inference_timecost.png")
    plt.close()
    ...
    plt.savefig(f"results/Reinforce/inference_hitrate.png")
    plt.close()

    np.save("results/Reinforce/reward.npy", reward_list)
    np.save("results/Reinforce/timecost.npy", time_list)
    np.save("results/Reinforce/hitrate.npy", hitrate_list)
    return 
...
if __name__ == '__main__':
    ...
    if not inference_problem:
        ...
        directory = f"results/Reinforce/checkpoint.pt"
        ckpt_dict = torch.load(directory)
        agent.policy_network.load_state_dict(ckpt_dict["agent"])
        # agent.value_network.load_state_dict(ckpt_dict["agent"])
