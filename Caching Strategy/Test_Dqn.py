...
class DQN:
    ...
    def _train(self):
        ...
        for _ in range(self.epoch):
            ...
            # 前向传播
            q_values = self.model(obs) # 当前状态下的Q值
            with torch.no_grad():
                next_q_values = self.next_model(next_obs) # 下一个状态的Q值
                
            # 计算时间差分损失
            q_values = q_values.gather(dim=1, index=action.unsqueeze(-1)).squeeze(-1) # 当前状态下采取动作的Q值
            q_target_values = reward + (1 - over) * self.gamma * torch.max(next_q_values, dim=-1).values
...
def inference(agent):
    ...
    plt.ylabel('Reward')
    plt.savefig(f"results/Dqn/inference.png")
    plt.close()
    ...
    plt.xlabel('Time Stamp')
    plt.ylabel('Timecost')
    plt.savefig(f"results/Dqn/inference_timecost.png")
    plt.close()
    ...
    plt.savefig(f"results/Dqn/inference_hitrate.png")
    plt.close()

    np.save("results/Dqn/reward.npy", reward_list)
    np.save("results/Dqn/timecost.npy", time_list)
    np.save("results/Dqn/hitrate.npy", hitrate_list)
...
if __name__ == '__main__':
    ...
    if not inference_problem:
        ...
        #初始化 DQN 模型
        dqn = DQN(3 * num_items + 1, num_items)
        ...
        # 加载训练好的模型并进行推理
        dqn = DQN(3 * num_items + 1, num_items)
        directory = f"results/Dqn/checkpoint.pt"
